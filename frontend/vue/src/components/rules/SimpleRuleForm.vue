<template>
  <div class="space-y-4">
    <div class="space-y-1">
      ...
      <input
        v-model="form.name"
        type="text" :placeholder="t('rules.simpleSection.namePlaceholder')"
        class="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[var(--accent)]"
      />
    </div>

    <div class="space-y-1">
      <label class="text-xs text-[var(--muted)] uppercase tracking-wide">{{ t('rules.simpleSection.seerrUser') }}</label>
      <div v-if="loadingUsers" class="text-xs text-[var(--muted)] py-2 flex items-center gap-2">
        <i class="fas fa-spinner fa-spin text-[10px]" /> {{ t('rules.simpleSection.loadingUsers') }}
      </div>
      <div v-else-if="users.length" class="space-y-1.5">
        <input
          v-model="userSearch"
          type="text"
          :placeholder="t('rules.simpleSection.searchUsers')"
        ...
        <div class="max-h-40 overflow-y-auto space-y-0.5 rounded-lg border border-[var(--border)] bg-[var(--bg3)]">
          ...
          <div v-if="!filteredUsers.length" class="text-xs text-[var(--muted)] text-center py-3">{{ t('common.noResults') }}</div>
        </div>
        <div v-if="form.seerr_user_id" class="text-xs text-[var(--muted)]">
          {{ t('rules.simpleSection.selected') }} <span class="text-[var(--text)] font-medium">{{ form.seerr_username }}</span>
        </div>
      </div>
      <div v-else class="text-xs text-[var(--muted)] py-2">
        {{ t('rules.simpleSection.noUsers') }}
      </div>
    </div>
    ...
    <div class="space-y-1">
      <label class="text-xs text-[var(--muted)] uppercase tracking-wide">
        {{ t('rules.simpleSection.discordIdOptional') }}
      </label>
      <input
        v-model="form.discord_id"
        type="text" :placeholder="t('rules.discordIdPlaceholder')"
        class="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[var(--accent)]"
      />
    </div>
