...
async function loadJobs() {
  try {
    ...
    document.getElementById('scheduler-cards').innerHTML=visibleJobs.map(j=>{
      ...
      const label=j.id==='scan_job'?t('Scan bibliothèques'):t('Vérification suppressions');
      ...
      return `<div class="card" style="padding:16px;display:flex;align-items:center;gap:14px">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:11px;color:var(--muted);margin-top:2px">${nxt ? `${nxt.toLocaleString('fr-FR', {day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})} — ${_fmtCountdown(diff)}` : t('Non planifié')}</div>
        </div>
        <button class="btn btn-ghost" style="padding:6px 10px;font-size:12px" onclick="${j.id==='scan_job'?'triggerScan':'triggerDeletion'}()"><i class="fas fa-play"></i>${t('Lancer')}</button>
      </div>`;
    }).join('');
    const tbody=document.getElementById('jobs-tbody');
    if (!history.length) { tbody.innerHTML=`<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">${t('Aucun historique')}</td></tr>`; return; }
    const seen=new Set();
    ...
    tbody.innerHTML=deduped.map(j=>{
      ...
      const lbl=jn==='scan'||jn==='scan_library'?`<i class="fas fa-magnifying-glass" style="color:var(--muted)"></i> ${t('Scan')}`:`<i class="fas fa-trash-can" style="color:var(--muted)"></i> ${t('Vérification')}`;
      const JOB_STATUS_LABELS = {success:'Succès', error:'Erreur', interrupted:'Interrompu', running:'En cours...'};
      const statusLabel = t(JOB_STATUS_LABELS[j.status] || j.status);
      return `<tr><td style="color:#e2e8f0">${lbl}</td><td style="color:var(--muted)">${start}</td><td style="color:var(--muted)">${end}</td><td style="color:var(--muted)">${dur}</td><td><span class="badge badge-${dc}">${statusLabel}</span></td><td style="color:var(--muted);font-size:12px">${j.result||'—'}</td></tr>`;
    }).join('');
...
async function triggerScan() {
  ...
  await api('/api/scan/trigger','POST'); toast(t('Scan démarré'),'info');
  setTimeout(()=>{ if(currentPage==='jobs') loadJobs(); },800);
}
async function triggerDeletion() {
  try { const st=await api('/api/media/job-status'); if(st.deletion_running){toast(t('Une vérification est déjà en cours'),'warn');return;} } catch(e){}
  await api('/api/deletion/trigger','POST'); toast(t('Vérification lancée'),'info');
  setTimeout(()=>{ if(currentPage==='jobs') loadJobs(); },800);
}
