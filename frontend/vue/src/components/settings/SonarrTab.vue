  ...
  <div class="space-y-3">
    ...
    >
      ...
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-lg bg-[#35C5F4]/20 flex items-center justify-center shrink-0">
      ...
      <div>
        ...
        <input v-model="srv.url" type="url" :placeholder="t('settings.arr.sonarr.urlPlaceholder')" class="field font-mono" @input="sync" />
      </div>
      ...
      <div>
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            @click="toggleServerKey(srv, 'sonarr_servers')"
          >
    ...
    <button
      ...
      @click="addServer"
    >
      <i class="fas fa-plus mr-2" />{{ t('settings.arr.sonarr.add') }}
    </button>
  </div>
</template>
...
function parseServers({ legacyFallback = false } = {}) {
  try {
    const raw = JSON.parse(props.form.sonarr_servers || '[]')
    if (Array.isArray(raw) && raw.length) return _decorate(raw)
  } catch (_e) { /* JSON parse failure — use fallback */ }
  if (legacyFallback && props.form.sonarr_url) {
    return _decorate([{
      id: 'legacy', name: 'Sonarr', url: props.form.sonarr_url,
      api_key: props.form.sonarr_api_key || '', enabled: true,
    }])
  }
  return []
}
...
function sync() {
  ...
  props.form.sonarr_servers = JSON.stringify(clean)
  nextTick(() => { _selfUpdate = false })
}

function addServer() {
  servers.value.push({
    id: `sonarr-${Date.now()}`, name: 'Sonarr', url: '', api_key: '', enabled: true,
...
function removeServer(idx) {
  ...
  if (servers.value.length === 0) {
    props.form.sonarr_url = ''
    props.form.sonarr_api_key = ''
  }
  sync()
}
...
async function testInstance(idx) {
  ...
  try {
    const { data } = await api.post('/settings/test-arr', {
      type: 'sonarr', url: srv.url, api_key: srv.api_key,
    })
...
watch(() => props.form.sonarr_servers, () => {
