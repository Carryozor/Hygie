...
async def test_insert_and_get_pending_queue(db_path):
    await insert_queue_entry(_ENTRY)
    rows = await get_pending_queue()
    assert len(rows) == 1
...
async def test_get_queued_and_ignored_ids(db_path):
    await insert_queue_entry(_ENTRY)
    queued, ignored = await get_queued_and_ignored_ids()
    assert "e1" in queued
...
async def test_mark_notified_detected(db_path):
    await insert_queue_entry(_ENTRY)
    await mark_notified_detected("e1")
...
async def test_update_queue_status(db_path):
    ...
    rows = await get_pending_queue()
    item_id = rows[0]["id"]
    await update_queue_status(item_id, "deleted")
    async with aiosqlite.connect(db_path) as db:
        async with db.execute(
...
async def test_get_enabled_libraries(db_path):
    async with aiosqlite.connect(db_path) as db:
        ...
        await db.commit()
    libs = await get_enabled_libraries("0")
    assert len(libs) == 1
